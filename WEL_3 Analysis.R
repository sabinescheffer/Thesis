...
WEL_3$DAT_ENV_CMU <- as.POSIXct(WEL_3$DAT_ENV_CMU, format = "%d%b%Y")
WEL_3$DAT_ENV_CMU <- format(WEL_3$DAT_ENV_CMU, "%d-%m-%y %H:%M")

#Calculating time interval between sending and clicking
times<- (difftime(as.POSIXct(WEL_3$DAT_1ER_CLK, format = "%d-%m-%y %H:%M", tz=Sys.timezone()), as.POSIXct(WEL_3$DAT_ENV_CMU, "%d-%m-%y %H:%M", tz=Sys.timezone()), units = 'mins'))-365
...
# Split hazard models: some fraction of the population will never click
split_exponential_cov<- flexsurvcure(sobj~WEL_3$FB_TIER + WEL_3$NBR_OUV, data=WEL_3, link="logistic", dist="exp", mixture=T, log=T)
print(split_exponential_cov)
BIC(split_exponential_cov)

split_weibull_cov <- flexsurvcure(sobj~WEL_3$FB_TIER + WEL_3$NBR_OUV, data=WEL_3, link="logistic", dist="weibullPH", mixture=T, log=T)
print(split_weibull_cov)
BIC(split_weibull_cov)

split_loglogistic_cov<- flexsurvcure(sobj~WEL_3$FB_TIER + WEL_3$NBR_OUV, data=WEL_3, link="logistic", dist="llogis", mixture=T, log=T)
print(split_loglogistic_cov)
BIC(split_loglogistic_cov)

split_lognormal_cov <- flexsurvcure(sobj~~WEL_3$FB_TIER + WEL_3$NBR_OUV, data=WEL_3, link="logistic", dist="lnorm", mixture=T, log=T)
...
plot(km_test, xlab="Time (minutes)", ylab="Survival Probability", main="Best Fitting models for WEL_3", conf.int = FALSE, ylim=c(0.4,1.0), width=880, height=710, res=300, family="serif")
