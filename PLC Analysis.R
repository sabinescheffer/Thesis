...
# Split hazard models: some fraction of the population will never click
split_exponential_cov<- flexsurvcure(sobj~PLC$FB_TIER + PLC$NBR_OUV, data=PLC, link="logistic", dist="exp", mixture=T, log=T)
print(split_exponential_cov)
BIC(split_exponential_cov)

split_weibull_cov <- flexsurvcure(sobj~PLC$FB_TIER + PLC$NBR_OUV, data=PLC, link="logistic", dist="weibullPH", mixture=T, log=T)
print(split_weibull_cov)
BIC(split_weibull_cov)

split_loglogistic_cov<- flexsurvcure(sobj~PLC$FB_TIER + PLC$NBR_OUV, data=PLC, link="logistic", dist="llogis", mixture=T, log=T)
print(split_loglogistic_cov)
BIC(split_loglogistic_cov)

split_lognormal_cov <- flexsurvcure(sobj~~PLC$FB_TIER + PLC$NBR_OUV, data=PLC, link="logistic", dist="lnorm", mixture=T, log=T)
